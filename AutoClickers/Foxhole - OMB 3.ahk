...
T := !T
While (T) {
	ControlClick, X%xpos% Y%ypos%, OMB 3, , Left, 1
	sleep, 100
}
return
...
T := !T
While (T) {
	PostMessage, 0x0200, 0, cX&0xFFFF | cY<<16,, OMB 3 ; WM_MOVEMOUSE
	PostMessage, 0x201, 0, cX&0xFFFF | cY<<16,, OMB 3 ; WM_LBUTTONDOWN  
  	PostMessage, 0x202, 0, cX&0xFFFF | cY<<16,, OMB 3 ; WM_LBUTTONUP  
	sleep, 100
}
return
...
T := !T
While (T) {
	ControlClick, X%xpos% Y%ypos%, OMB 3, , Left, 1, D
}
ControlClick, X%xpos% Y%ypos%, OMB 3, , Left, 1, u
return
...
T := !T
While (T) {
	ControlClick, X%xpos% Y%ypos%, OMB 3, , Right, 1, D
}
ControlClick, X%xpos% Y%ypos%, OMB 3, , Right, 1, u
return
...
SETYOURKEYHERE::
toggle := !toggle
ControlSend,,{w down}, OMB 3
if (toggle) {
	SetTimer, PressW, 1000
}	else {
	SetTimer, PressW, Off
	ControlSend,,{w up}, OMB 3
}
return
...
SETYOURKEYHERE::
toggle := !toggle
ControlSend,,{s down}, OMB 3
if (toggle) {
	SetTimer, PressS, 1000
}	else {
	SetTimer, PressS, Off
	ControlSend,,{s up}, OMB 3
}
return
